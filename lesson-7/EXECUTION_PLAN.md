# План виконання завдання Lesson-7

## 📋 Огляд завдання

Створити кластер Kubernetes у тій самій мережі (VPC), яку ви налаштували в попередньому домашньому завданні, та реалізувати такі компоненти:

1. ✅ Створення кластера Kubernetes через Terraform
2. ✅ Налаштування Elastic Container Registry (ECR) для зберігання Docker-образу вашого Django-застосунку
3. ✅ Завантаження Docker-образу Django до ECR
4. ✅ Створення helm chart (deployment.yaml, service.yaml, hpa.yaml, configmap.yaml)
5. ✅ Перенесення змінних середовища (env) з теми 4 в ConfigMap

## ✅ Виконані компоненти

### 1. Створення кластера Kubernetes через Terraform
- **Файл**: `modules/eks/eks.tf`
- **Компоненти**:
  - EKS кластер з версією 1.28
  - IAM ролі для кластера та групи вузлів
  - Security Groups для кластера та вузлів
  - Група вузлів з типом t3.medium (2-4 вузли)

### 2. Налаштування ECR
- **Файл**: `modules/ecr/ecr.tf` (успадковано з lesson-5)
- **Компоненти**:
  - ECR репозиторій lesson-7-ecr
  - Автоматичне сканування образів
  - Політика життєвого циклу
  - Політика доступу

### 3. Django застосунок
- **Файли**: `django-app/`
- **Компоненти**:
  - Django 4.2.7 проект
  - Gunicorn WSGI сервер
  - Health check endpoint (/health/)
  - Dockerfile для контейнеризації
  - Змінні середовища через python-decouple

### 4. Helm Charts
- **Файли**: `charts/django-app/`
- **Компоненти**:
  - `deployment.yaml` - розгортання з health checks та ресурсними лімітами
  - `service.yaml` - LoadBalancer для зовнішнього доступу
  - `hpa.yaml` - Horizontal Pod Autoscaler (2-6 подів, >70% CPU/Memory)
  - `configmap.yaml` - змінні середовища
  - `values.yaml` - конфігураційні параметри

### 5. Змінні середовища в ConfigMap
- **Файл**: `charts/django-app/templates/configmap.yaml`
- **Змінні**:
  - DEBUG, SECRET_KEY, ALLOWED_HOSTS
  - STATIC_URL, STATIC_ROOT, LOG_LEVEL
  - ENVIRONMENT

## 🚀 Кроки для виконання

### Етап 1: Підготовка інфраструктури
```bash
cd lesson-7

# Ініціалізація Terraform
terraform init

# Перегляд плану
terraform plan

# Застосування змін (створення EKS кластера)
terraform apply
```

### Етап 2: Налаштування kubectl
```bash
# Налаштування kubectl для роботи з EKS кластером
./scripts/setup-kubectl.sh
```

### Етап 3: Збірка та завантаження Docker образу
```bash
# Збірка Django застосунку та завантаження в ECR
./scripts/build-and-push.sh
```

### Етап 4: Розгортання застосунку
```bash
# Розгортання через Helm
./scripts/deploy.sh
```

## 🔍 Перевірка виконання

### Перевірка інфраструктури
```bash
# Перевірка EKS кластера
aws eks describe-cluster --name lesson-7-eks-cluster --region us-west-2

# Перевірка ECR репозиторію
aws ecr describe-repositories --repository-names lesson-7-ecr --region us-west-2
```

### Перевірка Kubernetes ресурсів
```bash
# Перевірка подів
kubectl get pods -l app.kubernetes.io/name=django-app

# Перевірка сервісів
kubectl get svc django-app

# Перевірка HPA
kubectl get hpa django-app

# Перевірка ConfigMap
kubectl get configmap django-app-config
```

### Перевірка застосунку
```bash
# Отримання URL LoadBalancer
kubectl get svc django-app -o jsonpath='{.status.loadBalancer.ingress[0].hostname}'

# Тестування health check
curl http://[load-balancer-url]/health/
```

## 📊 Очікувані результати

### Після виконання terraform apply:
- ✅ EKS кластер lesson-7-eks-cluster створено
- ✅ ECR репозиторій lesson-7-ecr створено
- ✅ VPC з публічними та приватними підмережами налаштовано

### Після виконання build-and-push.sh:
- ✅ Docker образ Django завантажено в ECR
- ✅ Образ доступний для використання в Kubernetes

### Після виконання deploy.sh:
- ✅ Django застосунок розгорнуто в Kubernetes
- ✅ LoadBalancer створено та налаштовано
- ✅ HPA активовано для автоматичного масштабування
- ✅ ConfigMap з змінними середовища створено

## 🧪 Тестування

### Локальне тестування Django застосунку
```bash
cd django-app
./test-local.sh
```

### Тестування в Kubernetes
```bash
# Перевірка логів
kubectl logs -l app.kubernetes.io/name=django-app

# Тестування масштабування
kubectl scale deployment django-app --replicas=4
kubectl get hpa django-app
```

## 🧹 Очищення

### Видалення застосунку
```bash
helm uninstall django-app
```

### Видалення інфраструктури
```bash
terraform destroy
```

## ⚠️ Важливі зауваження

1. **AWS Account ID**: Замініть `123456789012` в `charts/django-app/values.yaml`
2. **Вартість**: EKS кластер та LoadBalancer є платними ресурсами
3. **Безпека**: Змініть SECRET_KEY для production
4. **Регіон**: За замовчуванням використовується us-west-2

## 📚 Додаткові ресурси

- [Terraform AWS Provider](https://registry.terraform.io/providers/hashicorp/aws/latest/docs)
- [Amazon EKS](https://docs.aws.amazon.com/eks/)
- [Helm Documentation](https://helm.sh/docs/)
- [Kubernetes Documentation](https://kubernetes.io/docs/) 