# Швидкий старт - Lesson-7

Цей гід допоможе вам швидко розгорнути повну інфраструктуру з Django застосунком на AWS.

## 🚀 Швидкий старт (5 кроків)

### Крок 1: Підготовка
```bash
# Переконайтеся, що у вас встановлено необхідні інструменти
aws --version
terraform --version
kubectl version --client
helm version
docker --version
```

### Крок 2: Створення інфраструктури
```bash
cd lesson-7

# Ініціалізація та розгортання інфраструктури
terraform init
terraform plan
terraform apply
```

### Крок 3: Налаштування kubectl
```bash
# Налаштування kubectl для роботи з EKS кластером
./scripts/setup-kubectl.sh
```

### Крок 4: Збірка та завантаження Docker образу
```bash
# Збірка та завантаження Django застосунку в ECR
./scripts/build-and-push.sh
```

### Крок 5: Розгортання застосунку
```bash
# Розгортання Django застосунку через Helm
./scripts/deploy.sh
```

## ✅ Перевірка роботи

### Отримання URL застосунку
```bash
kubectl get svc django-app -o jsonpath='{.status.loadBalancer.ingress[0].hostname}'
```

### Перевірка статусу
```bash
# Перевірка подів
kubectl get pods -l app.kubernetes.io/name=django-app

# Перевірка сервісів
kubectl get svc django-app

# Перевірка HPA
kubectl get hpa django-app
```

### Перегляд логів
```bash
kubectl logs -l app.kubernetes.io/name=django-app
```

## 🧹 Очищення

### Видалення застосунку
```bash
helm uninstall django-app
```

### Видалення інфраструктури
```bash
terraform destroy
```

## 🔧 Локальне тестування

Для тестування Django застосунку локально:

```bash
cd django-app
./test-local.sh
```

Застосунок буде доступний за адресою: http://localhost:8000

## 📋 Важливі URL

- **Django застосунок**: http://[load-balancer-url]
- **Health check**: http://[load-balancer-url]/health/
- **ECR репозиторій**: [aws-account-id].dkr.ecr.us-west-2.amazonaws.com/lesson-7-ecr

## ⚠️ Важливі зауваження

1. **AWS Account ID**: Замініть `123456789012` в `charts/django-app/values.yaml` на ваш AWS Account ID
2. **Регіон**: За замовчуванням використовується `us-west-2`
3. **Вартість**: EKS кластер та LoadBalancer є платними ресурсами
4. **Безпека**: Змініть `SECRET_KEY` в values.yaml для production

## 🆘 Вирішення проблем

### Проблема: kubectl не може підключитися до кластера
```bash
aws eks update-kubeconfig --region us-west-2 --name lesson-7-eks-cluster
```

### Проблема: Docker не може завантажити образ в ECR
```bash
aws ecr get-login-password --region us-west-2 | docker login --username AWS --password-stdin [aws-account-id].dkr.ecr.us-west-2.amazonaws.com
```

### Проблема: Helm не може знайти чарт
```bash
# Перевірте чи ви знаходитесь в правильній директорії
pwd  # Повинно бути lesson-7
ls charts/django-app/  # Повинен показати файли чарту
``` 